...
# Ask for tunnel port
printf "Local port to forward [8080]: " >&2
read -r TUNNEL_PORT
...
echo "Opening SSH tunnel: localhost:$TUNNEL_PORT -> $NAME:$TUNNEL_PORT ..." >&2
exec ssh -N -L "$TUNNEL_PORT:localhost:$TUNNEL_PORT" "$SSH_USER@$IP" -p "$PORT" -i "$SSH_KEY"
